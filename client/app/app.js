...
.run(function ($rootScope, $location, $state, $route, Auth, USER_ROLES) {
  ...
  $rootScope.$on('$stateChangeStart', function (event, next) {
    Auth.isLoggedInAsync(function(loggedIn) {
      ...
      if (!loggedIn) {
        ...
        if (next.hasRole && next.hasRole !== $rootScope.user.role) {
          $state.go('app');
        }
      }
    });
  ...
  function($rootScope, $scope, $state, $http, $timeout, Utils, USER_ROLES) {
    ...
    var isActive = function(item) {
      ...
      if( !item.sref || item.sref == '#') {
        ...
        return $state.is(item.sref, item.params) || $state.includes(item.sref, item.params);
    };
